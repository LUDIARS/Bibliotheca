...
function extractToken(c: Context): string | null {
  ...
  // PWA は fetch を Authorization 付きで投げるが、 リダイレクト直後など
  // cookie 経由のフォールバックも許可する (`Cernere-Token` cookie)。
...
// Hono の型に lazy に identity を流し込む
declare module 'hono' {
...
export async function _testOnlyVerify(token: string): Promise<AuthIdentity | null> {
  return verifyToken(token);
}

// next is unused but keep signature compatible for future composition
void (async () => {
  const _placeholder: Next = async () => {};
  void _placeholder;
})();
